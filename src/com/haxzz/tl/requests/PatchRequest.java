...
public class PatchRequest {
	...
	public PatchRequest(byte[] buffer) {
		try {
			...
			//read game
			Game = input.readShort();
					
			//close buffer
			input.close();	

			//write packet
			if(TamperedListener.getWritePackets()) Utilities.WritePacketBinary("PatchRequest.bin", CPUKey, buffer);
			
		} catch (IOException e) {
			Logger.error(e, "PatchRequest");
		}
	}
